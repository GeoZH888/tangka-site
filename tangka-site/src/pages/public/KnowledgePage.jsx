...
export default function KnowledgePage() {
  ...
  return (
    <div className="kpage manuscript-page">
      <header className="kpage__head container">
        ...
        <HorizontalRule />
        <p className="kpage__intro reading">
      ...
      <div className="container">
        {Object.entries(byCategory).map(([cat, items]) => (
          <section key={cat} className="kpage__section">
            <h2 className="kpage__cat">{categoryLabel(cat)}</h2>
            <div className="kpage__grid">
              {items.map((a) => (
                <Link key={a.id || a.slug} to={`/knowledge/${a.slug}`} className="karticle">
                  ...
                  <h3 className={lang === 'zh' ? 'cn-title' : ''}>{t(a, 'title')}</h3>
                  <p>{t(a, 'excerpt')}</p>
